...
import { firstDifferenceInNode, applyDifferenceMarker, firstDifferenceInText } from "./differ";
...
test('detect nothing if inputs are the same', () => {
    ...
    const diffPoint = firstDifferenceInNode(oldBody, newBody);
...
test('detect missing child element', () => {
    ...
    expect(diffPoint.newNode).toBe(newBody.getElementsByTagName('b')[0]);
});
...
test('detect added child element', () => {
    ...
    const newBody = new JSDOM('<html><head></head><body><a><b><c></c></b></a></body></html>').window.document.getElementsByTagName('body')[0];
    const diffPoint = firstDifferenceInNode(oldBody, newBody);
    if (diffPoint == null) {
        throw 'Cannot be null';
    }
...
test('detect replaced child element', () => {
    ...
    expect(diffPoint.newNode).toBe(newBody.getElementsByTagName('X')[0]);
});
...
test('detect different text node', () => {
    ...
    expect(diffPoint.newNode).toBe(newBody.getElementsByTagName('b')[1].childNodes[0]);
});
...
test('detect element with different attributes', () => {
    ...
    expect(diffPoint.newNode).toBe(newBody.getElementsByTagName('b')[1]);
});
