...
export class BookmarkExtension implements Extension {
    ...
    public process(markdownIt: MarkdownIt, tokens: Token[], tokenIdx: number, context: ExtensionContext): void {
        ...
        switch (token.type) {
            ...
            case 'bm-e': {
                const broken = breakOnSlashes(token.content);
                if (broken.length !== 3) {
                    throw 'Advanced bookmark tags require exactly 3 parameters separated by slashes: error_text/regex/regex_flags';
                }
    ...
    public render(markdownIt: MarkdownIt, tokens: Token[], tokenIdx: number, context: ExtensionContext): string {
        ...
        if (bookmarkEntry === undefined || bookmarkEntry instanceof NormalBookmarkEntry === false) {
            throw 'Undefined/incorrect bookmark when rendering: ' + anchorId; // this should never happen
        }
        const normalBookmarkEntry = bookmarkEntry as NormalBookmarkEntry;
...
export class BookmarkScannerList<T> {
    ...
    public scan(text: string): ListScanResult<T> | null {
        ...
        const shortestMatchIdx = filterMatches[0].capture.captureMatch.length;
        filterMatches = filterMatches.filter(m => m.capture.captureMatch.length === shortestMatchIdx);
